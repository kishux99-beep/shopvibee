...
function DealSkeleton() {
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col animate-pulse">
      <div className="aspect-video bg-gray-200 dark:bg-gray-800 w-full" />
      <div className="p-4 flex flex-col flex-1 justify-between space-y-3">
        ...
        <div className="h-9 bg-gray-200 dark:bg-gray-800 rounded-xl w-full mt-4" />
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  // Top 10 Deals (Taking first 10 items from initialDeals)
  const top10Deals = initialDeals.slice(0, 10);

  // ⚡ Flash Deals (Taking items for flash sales)
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 font-sans flex flex-col justify-between transition-colors duration-300 relative">
      
      <div>
        {/* 🚀 Prominent Toast Notification Pop-up */}
        {toastMessage && (
          <div className="fixed bottom-6 right-6 z-50 bg-indigo-600 dark:bg-indigo-500 text-white px-5 py-3.5 rounded-2xl shadow-2xl flex items-center gap-3 text-xs sm:text-sm font-bold animate-bounce transition-all border border-indigo-400/30">
        ...
        <header className="sticky top-0 z-40 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md shadow-sm border-b border-gray-100 dark:border-gray-800">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-col sm:flex-row items-center gap-3 sm:gap-4">
            ...
            <div className="w-full sm:w-auto flex items-center justify-between">
              ...
              >
                <Image
                  ...
                  priority
                />
                <span className="text-[10px] sm:text-xs font-semibold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 px-2 py-0.5 rounded-full border border-indigo-100 dark:border-indigo-800">
                  Deals
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 sm:hidden">
                {/* 🔔 Mobile Get Alerts / Subscribed Button */}
                <button
                  ...
                  className={`p-2 rounded-full transition active:scale-95 ${
                    isSubscribed 
                      ...
                      : 'bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800'
                  }`}
                ...
                <button
                  ...
                  className={`p-2 rounded-full relative transition active:scale-95 ${
                    showWishlistOnly ? 'bg-rose-100 dark:bg-rose-950 text-rose-600' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200'
                  }`}
                >
                  💖
                  {wishlist.length > 0 && (
                ...
                {mounted && (
                  <button
                    ...
                    className="p-2 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 transition active:scale-95"
                  >
                    {theme === 'dark' ? '☀️' : '🌙'}
                  </button>
                )}
            ...
            {/* Live Search Input with Amazon-style Auto-Suggest Dropdown */}
            <div className="w-full flex-1 relative" ref={searchContainerRef}>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-gray-100 dark:bg-gray-800 text-sm border-none rounded-full pl-9 sm:pl-10 pr-10 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 dark:text-white transition shadow-inner"
                />
                {searchQuery && (
                  <button
                    onClick={() => setSearchQuery('')}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-xs bg-gray-200 dark:bg-gray-700 rounded-full w-5 h-5 flex items-center justify-center transition"
                  >
                    ✕
                  </button>
                )}
              ...
              {/* 🛒 Amazon-style Auto-Suggest Dropdown */}
              {isSearchFocused && searchSuggestions.length > 0 && (
                <div className="absolute left-0 right-0 mt-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-2xl z-50 overflow-hidden divide-y divide-gray-100 dark:divide-gray-800">
                  <div className="px-4 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider bg-gray-50 dark:bg-gray-950">
                  ...
                  {searchSuggestions.map((item) => (
                    <div
                      ...
                      className="px-4 py-3 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 cursor-pointer flex items-center justify-between transition group"
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-gray-400 text-xs">🔍</span>
                        <div>
                          <p className="text-xs sm:text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 line-clamp-1">
                            {item.title}
                          </p>
            ...
            <div className="hidden sm:flex items-center gap-3">
              ...
              >
                <span>💖 Saved</span>
                <span className="bg-rose-100 dark:bg-rose-950 text-rose-600 dark:text-rose-300 text-xs px-2 py-0.2 rounded-full font-bold">
                  {wishlist.length}
                </span>
              </button>
              ...
              {/* Dynamic Alerts / Subscribed Button */}
              {isSubscribed ? (
                <button 
                  onClick={() => setIsAlertsModalOpen(true)}
                  className="bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-300 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 text-sm font-semibold px-4 py-2 rounded-full hover:bg-emerald-100 dark:hover:bg-emerald-900 transition active:scale-95 flex items-center gap-1.5"
                >
                  ...
                  <span className="text-xs text-emerald-600 dark:text-emerald-400 underline ml-0.5">(Edit)</span>
                </button>
              ) : (
                <button 
                  onClick={() => setIsAlertsModalOpen(true)}
                  className="bg-indigo-600 text-white text-sm font-medium px-4 py-2 rounded-full hover:bg-indigo-700 transition active:scale-95 whitespace-nowrap shadow-sm shadow-indigo-200 dark:shadow-none"
                >
                  Get Alerts
                </button>
              )}
        ...
        {/* Hero Banner */}
        <section className="bg-gradient-to-r from-indigo-600 to-violet-600 dark:from-indigo-900 dark:to-violet-950 text-white py-8 sm:py-12 px-4 text-center transition-all">
          <h1 className="text-2xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight">
            Today's Top Curated Deals
          </h1>
          <p className="mt-2 sm:mt-3 text-indigo-100 dark:text-indigo-200 max-w-xl mx-auto text-xs sm:text-base">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          {/* ⚡ 1. FLASH DEALS SECTION WITH COUNTDOWN TIMER */}
                ...
                >
                  {flashDeals.map((deal) => (
                    <div
                      ...
                      className="min-w-[260px] max-w-[260px] sm:min-w-[280px] sm:max-w-[280px] bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 rounded-2xl overflow-hidden border border-white/20 shadow-lg hover:shadow-2xl transition-all duration-300 flex flex-col cursor-pointer group/card shrink-0 hover:-translate-y-1 relative"
                    >
                      ...
                      <div className="p-3.5 flex flex-col flex-1 justify-between">
                        <div>
                          ...
                          <h4 className="font-semibold text-gray-900 dark:text-gray-100 text-xs sm:text-sm mt-0.5 line-clamp-1 group-hover/card:text-rose-600 transition">
                            {deal.title}
                          </h4>
                          <div className="flex items-baseline gap-2 mt-2">
                            <span className="text-sm sm:text-base font-bold text-gray-900 dark:text-white">{deal.price}</span>
          ...
          {/* 🚀 🔥 TODAY'S TOP 10 DEALS SECTION */}
          {!showWishlistOnly && !searchQuery && selectedCategory === 'All' && (
            <div className="mb-10 bg-gradient-to-br from-amber-500/10 via-orange-500/5 to-transparent dark:from-amber-500/5 dark:via-transparent border border-amber-500/20 rounded-3xl p-4 sm:p-6 shadow-sm">
              ...
              {/* Horizontal Scroll with Amazon-style buttons */}
              <div className="relative group">
                <button
                  onClick={() => scrollTop10('left')}
                  className="absolute left-0 top-1/2 -translate-y-1/2 -ml-3 z-20 bg-white dark:bg-gray-800 text-gray-800 dark:text-white shadow-lg border border-gray-200 dark:border-gray-700 w-9 h-9 rounded-full flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700 transition active:scale-95 hidden sm:flex"
                  title="Scroll Left"
                >
                  &lt;
                </button>
                ...
                >
                  {top10Deals.map((deal, idx) => (
                    <div
                      ...
                      className="min-w-[260px] max-w-[260px] sm:min-w-[280px] sm:max-w-[280px] bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border border-gray-200/80 dark:border-gray-800 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col cursor-pointer group/card shrink-0 hover:-translate-y-1 relative"
                    >
                      <div className="relative aspect-video bg-gray-100 dark:bg-gray-800 overflow-hidden">
                        <img
                          src={deal.image}
                      ...
                      <div className="p-3.5 flex flex-col flex-1 justify-between">
                        <div>
                          ...
                          <h4 className="font-semibold text-gray-900 dark:text-gray-100 text-xs sm:text-sm mt-0.5 line-clamp-1 group-hover/card:text-indigo-600 transition">
                            {deal.title}
                          </h4>
                          <div className="flex items-baseline gap-2 mt-2">
                            <span className="text-sm sm:text-base font-bold text-gray-900 dark:text-white">{deal.price}</span>
                ...
                <button
                  ...
                  className="absolute right-0 top-1/2 -translate-y-1/2 -mr-3 z-20 bg-white dark:bg-gray-800 text-gray-800 dark:text-white shadow-lg border border-gray-200 dark:border-gray-700 w-9 h-9 rounded-full flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700 transition active:scale-95 hidden sm:flex"
                  title="Scroll Right"
                >
                  &gt;
                </button>
          ...
          {/* Category Filters with Amazon-style Left/Right Scroll Buttons */}
          <div className="relative mb-6 group">
            {/* Left Arrow Button */}
            <button
              onClick={() => scrollCategories('left')}
              className="absolute left-0 top-1/2 -translate-y-1/2 -ml-2 sm:-ml-3 z-20 bg-white dark:bg-gray-800 text-gray-800 dark:text-white shadow-lg border border-gray-200 dark:border-gray-700 w-9 h-9 rounded-full flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700 transition active:scale-95"
              title="Scroll Left"
            >
              &lt;
            </button>
            ...
            <button
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 -mr-2 sm:-mr-3 z-20 bg-white dark:bg-gray-800 text-gray-800 dark:text-white shadow-lg border border-gray-200 dark:border-gray-700 w-9 h-9 rounded-full flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700 transition active:scale-95"
              title="Scroll Right"
            >
              &gt;
            </button>
          ...
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            <h2 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white border-l-4 border-indigo-600 pl-2.5">
              {showWishlistOnly 
                ? 'Your Saved Wishlist 💖' 
                : searchQuery 
          ...
          {/* Deals Grid with Skeleton Loading */}
          {isLoading ? (
              ...
              {filteredDeals.map((deal) => {
                ...
                return (
                  <div 
                    ...
                    className="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-xl dark:hover:shadow-indigo-950/30 transition-all duration-300 flex flex-col cursor-pointer group hover:-translate-y-1 relative"
                  >
                    <div className="relative aspect-video bg-gray-100 dark:bg-gray-800 overflow-hidden">
                      <img
                        src={deal.image}
                    ...
                    <div className="p-4 flex flex-col flex-1 justify-between">
                      <div>
                        ...
                        <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm sm:text-base mt-0.5 line-clamp-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition">
                          {deal.title}
                        </h3>
                        <div className="flex items-baseline gap-2 mt-2 sm:mt-3">
                          <span className="text-base sm:text-lg font-bold text-gray-900 dark:text-white">{deal.price}</span>
                      ...
                      <button
                        ...
                        className="mt-4 block w-full text-center bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 dark:hover:bg-indigo-900 text-indigo-700 dark:text-indigo-300 font-semibold py-2 rounded-xl text-xs sm:text-sm transition border border-transparent dark:border-indigo-800 active:scale-95"
                      >
      ...
      {isAlertsModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-gray-900 rounded-3xl max-w-md w-full overflow-hidden shadow-2xl relative p-6 sm:p-8 border dark:border-gray-800">
            
            {/* Close Button */}
            <button 
              onClick={() => setIsAlertsModalOpen(false)}
              className="absolute top-4 right-4 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 w-8 h-8 rounded-full flex items-center justify-center transition"
            >
              ✕
            </button>

            <div className="text-center mb-5">
              <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 rounded-2xl mx-auto flex items-center justify-center text-xl mb-3 shadow-inner">
                🔔
              </div>
            ...
            <form onSubmit={handleSubscribeSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white transition"
                  required
                />
              </div>
              ...
              <button
                ...
                className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-bold py-3 rounded-xl shadow-lg shadow-indigo-200 dark:shadow-none transition text-sm active:scale-95 flex items-center justify-center gap-2"
              >
      ...
      {selectedDeal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-gray-900 rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col border dark:border-gray-800">
            ...
            <div className="relative h-56 sm:h-64 bg-gray-100 dark:bg-gray-800 shrink-0">
              <img 
            ...
            <div className="p-5 sm:p-6 overflow-y-auto flex-1">
              <div className="flex items-center justify-between text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
                <span className="uppercase tracking-wider text-indigo-600 dark:text-indigo-400 font-semibold">{selectedDeal.category}</span>
                <span className="bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 px-2 py-0.5 rounded font-medium">Store: {selectedDeal.store}</span>
              </div>

              <h2 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white">{selectedDeal.title}</h2>

              <div className="flex items-baseline gap-3 my-3">
                <span className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white">{selectedDeal.price}</span>
                <span className="text-base text-gray-400 line-through">{selectedDeal.originalPrice}</span>
              </div>

              {selectedDeal.promoCode && (
                <div className="my-3 p-3 bg-indigo-50 dark:bg-indigo-950/50 border border-dashed border-indigo-200 dark:border-indigo-800 rounded-2xl flex items-center justify-between">
                  <div>
                    <span className="text-[10px] uppercase tracking-wider text-indigo-500 dark:text-indigo-400 font-bold block">Extra Discount Code</span>
                    <span className="text-sm font-mono font-extrabold text-indigo-900 dark:text-indigo-200">{selectedDeal.promoCode}</span>
                  </div>
                  <button
              ...
              <div className="mt-4">
                <h4 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wide mb-2">Deal Highlights</h4>
                <ul className="space-y-1.5">
                  {selectedDeal.features.map((feature, idx) => (
                    <li key={idx} className="text-xs text-gray-600 dark:text-gray-300 flex items-center gap-2">
                      <span className="text-indigo-600 dark:text-indigo-400 font-bold">✓</span> {feature}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {/* Modal Footer with Share + Buy Now Buttons */}
            <div className="p-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 shrink-0 flex items-center gap-2">
              <button
                onClick={(e) => handleShareDeal(e, selectedDeal)}
                className="p-3 bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-2xl transition font-semibold text-sm flex items-center justify-center gap-1.5 active:scale-95"
                title="Share Deal"
              >
                <span>↗️</span>
              ...
              <a
                ...
                className="flex-1 text-center bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-2xl shadow-lg shadow-indigo-200 dark:shadow-none transition text-sm sm:text-base active:scale-95"
              >
      ...
      {/* Professional Footer */}
      <footer className="bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 pt-10 pb-6 mt-12 transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pb-8 border-b border-gray-100 dark:border-gray-800">
            ...
            <div>
              <h4 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-3">Quick Navigation</h4>
              <ul className="space-y-2 text-xs text-gray-600 dark:text-gray-400 font-medium">
                <li><button onClick={() => { setShowWishlistOnly(false); setSelectedCategory('All'); }} className="hover:text-indigo-600 dark:hover:text-indigo-400 transition">All Trending Deals</button></li>
                <li><button onClick={() => { setShowWishlistOnly(false); setSelectedCategory('Supplements'); }} className="hover:text-indigo-600 dark:hover:text-indigo-400 transition">Supplements & Whey Deals</button></li>
                <li><button onClick={() => setShowWishlistOnly(true)} className="hover:text-indigo-600 dark:hover:text-indigo-400 transition">Saved Wishlist (💖)</button></li>
              </ul>
            </div>

            <div>
              <h4 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-3">📜 Affiliate Disclosure</h4>
              <p className="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-100 dark:border-gray-800">
          ...
          <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-gray-400 dark:text-gray-500">
